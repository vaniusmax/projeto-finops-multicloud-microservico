...
const columnHelper = createColumnHelper<TopAccountsResponse[number]>();
...
export function LinkedAccountTable({ data, currency, title = "DADOS: LINKED ACCOUNT" }: LinkedAccountTableProps) {
  const rows = useMemo(() => data.slice(0, 10), [data]);
  const total = useMemo(() => rows.reduce((acc, row) => acc + row.total, 0), [rows]);

  const columns = [
    columnHelper.accessor("linkedAccount", {
